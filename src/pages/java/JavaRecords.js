...
export default function JdkRelases() {
    return (
        <Art artId="javaRecords">
            ...
            <Pa>
                Java records first came as Preview in JDK 14, with the intention of reducing boilerplate and mitigate developer error 
                while creating POJOs (Plain Old Java Objects)
                {' '}<Ref code="jdk14-record-jep" />. In practice it did not accomplish what it aimed to do to the extend I would personally desire, since
                ...
                Here we will go over some use cases, and hopefully demonstrate why that is the case. Furthermore, we see to which types of problemes records are most useful.
            </Pa>
            <T2>Using records</T2>
            <Pa>
            ...
            <Pa>
                Now the record could be used for example as the result of the method, for example:
            </Pa>
            <Java>{`
            ...
            <Pa> A very common pattern while writing production applications is the definition
                of DTOs (Data Transfer Objects), which usually contain a larger number of field than just a
                couple. In such case records become a little cumbersome:
            </Pa>
            <Java>
                {`
            ...
            <Pa>
                ...
                one has to peak into the record definition to check in what order the parameters were supposed to be passed. This
                is often solved by applying the builder pattern <Ref code="builder-pattern" />, which decouples instantiating
                from the places in the code where instantiation is needed, greatly increasing readability and reducing the probability of bugs being introduced.
            </Pa>
            <Pa>
                As you might have realised by now, Builders apply to POJOs too, so when it comes to the readability issue, we
            ...
            <Pa>For a software developer, building complex objects is daily routine, which means that <b>records are not a silver bullet</b>, and
                become even less compeling if we take into account that most of the time we are
                ...
                tuple unpacking, or inline anonimous definition of tuples, which have been present in many of the most widely used programming languages
